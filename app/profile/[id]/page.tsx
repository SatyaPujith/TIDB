...
import { ArrowLeft, Clock, FileText, Download, User, Calendar, MapPin, Award } from 'lucide-react';
import Link from 'next/link';
import { headers } from 'next/headers';
import dynamic from 'next/dynamic';

const DownloadProfilePDF = dynamic(() => import('@/components/DownloadProfilePDF'), { ssr: false });
...
export default async function ProfilePage({ params }: { params: { id: string } }) {
  ...
  const response = await fetch(`${baseUrl}/api/profiles/${params.id}`, { cache: 'no-store' });
  ...
  const stats = [
    ...
    {
      label: 'Categories',
      value: [...new Set(events.flatMap((e) => e.categories))].length,
      icon: Award,
    },
    {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          ...
          <DownloadProfilePDF profileId={profile.id} profileName={profile.name} />
        </div>
